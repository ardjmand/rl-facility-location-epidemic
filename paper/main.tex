...
\boxed{\;\mathcal{R}_0(L) \;=\; \frac{1}{\delta}\,\rho\!\left(\Lambda A\right)\;}
\end{equation}
...
Part~(i) follows from the fact that at any equilibrium with $\bm{I}^{\star} \neq \bm{0}$, the vector $\bm{I}^{\star}$ satisfies the fixed-point equation~\eqref{eq:endemic_fixedpoint} below, whose right-hand side is bounded above by $(\lambda_{\max}/\delta)\,(A\bm{I}^{\star})_i$ for small $\|\bm{I}^{\star}\|$.  When $\mathcal{R}_0(L) < 1$, this contraction prevents any nontrivial fixed point.

Part~(ii) follows from a bifurcation analysis at $\mathcal{R}_0(L) = 1$.  At this critical value, the Jacobian $\Lambda A - \delta I_n$ has a simple zero eigenvalue with a positive eigenvector (by Perron--Frobenius).  The forward nature of the bifurcation is verified for the homogeneous case in Proposition~\ref{prop:endemic_homogeneous}.  The positivity $I_i^{\star} > 0$ for all $i$ follows from the irreducibility of $A$: at any equilibrium with $\bm{I}^{\star} \geq \bm{0}$ and $\bm{I}^{\star} \neq \bm{0}$, the equation $\delta I_i = (\beta_1 S_i + \beta_2 V_i) \sum_{j \in \mathcal{N}_i} I_j$ and the connectivity of $\mathcal{G}$ imply $I_i > 0$ for every node.
\end{proof}
...
\[
b = \frac{k\,\bigl[\beta_1 \omega\, k(\beta_1 - \beta_2) + (\beta_1 \omega + \beta_2 \nu)(\beta_1 + \beta_2)\bigr]}{\nu + \omega},
\]
