...
Vaccination access is encoded through the rate
\[
\nu_i(L) \;=\; v_{\min} + (v_{\max}-v_{\min}) e^{-\alpha d_i(L)},
\]
where $v_{\max}$ and $v_{\min}$ denote the maximum and minimum achievable vaccination rates, corresponding to zero and infinite distance from a facility, respectively, $\alpha>0$ is a decay parameter, and
\[
d_i(L) \;=\; \min_{m\in L} \|z_i-m\|_2
\]
is the Euclidean distance from individual $i$ (located at $z_i\in\mathbb R^2$) to the closest open facility in the set $L$.  We adopt the convention $d_i(\emptyset)=+\infty$, so that $\nu_i(\emptyset)=v_{\min}$ when no facilities are open.
...
Assume $\nu_i(L)+\omega>0$ for all $i$ (in particular, this holds if $\omega>0$). 
Then the mean-field system~\eqref{eq:meanfield} admits a unique \ac{DFE} given by
...
\begin{lemma}[Properties of $\lambda_i$] \label{lem:lambda_properties}
Assume $\beta_1>\beta_2>0$ and $\nu_i,\omega\ge 0$. Then
\begin{equation}\label{eq:lambda_convex_combo}
\lambda_i \;=\; \beta_2 + (\beta_1-\beta_2)\,\frac{\omega}{\nu_i+\omega},
\end{equation}
so $\beta_2 \le \lambda_i \le \beta_1$. Moreover, whenever $\nu_i+\omega>0$,
\begin{align}
\frac{\partial \lambda_i}{\partial \omega}
&= \frac{(\beta_1-\beta_2)\,\nu_i}{(\nu_i+\omega)^2}\;\ge\;0, \label{eq:dlam_domega}\\
\frac{\partial \lambda_i}{\partial \beta_1}
&= \frac{\omega}{\nu_i+\omega} \;=\; S_i^\star,\label{eq:dlam_db1}\\
\frac{\partial \lambda_i}{\partial \beta_2}
&= \frac{\nu_i}{\nu_i+\omega} \;=\; V_i^\star.\label{eq:dlam_db2}
\end{align}
\end{lemma}

\begin{proof}
Equation~\eqref{eq:lambda_convex_combo} follows by rewriting~\eqref{eq:lambda_i} as
\[
\lambda_i=\frac{\beta_1\omega+\beta_2\nu_i}{\nu_i+\omega}
=\beta_2+\frac{(\beta_1-\beta_2)\omega}{\nu_i+\omega}.
\]
Since $\nu_i,\omega\ge 0$, we have $0\le \omega/(\nu_i+\omega)\le 1$, which implies $\beta_2\le \lambda_i\le \beta_1$.
The derivatives follow from direct differentiation of~\eqref{eq:lambda_i}.
\end{proof}
...
\begin{proposition}[Degree-weighted bounds] \label{prop:R0_degree_bounds}
Let $k_i = \sum_{j=1}^n a_{ij}$ denote the degree of node $i$ and define the row sums of $\Lambda A$ by
$r_i(L) = \sum_{j=1}^n (\Lambda A)_{ij} = \lambda_i(L)\,k_i$.
If $\mathcal G$ is connected (so $\Lambda A$ is irreducible and nonnegative), then
\begin{equation}\label{eq:R0_degree_bounds}
\frac{1}{\delta}\,\min_i \lambda_i(L)\,k_i
\;\le\;
\mathcal R_0(L)
\;\le\;
\frac{1}{\delta}\,\max_i \lambda_i(L)\,k_i.
\end{equation}
\end{proposition}

\begin{proof}
For a nonnegative matrix $M$, $\rho(M)\le \|M\|_\infty$, where $\|\cdot\|_\infty$ is the maximum absolute row-sum norm.  Since $\Lambda A$ is nonnegative, 
\[
\rho(\Lambda A)\le \|\Lambda A\|_\infty = \max_i \sum_{j=1}^n (\Lambda A)_{ij} = \max_i \lambda_i k_i.
\]
For the lower bound, apply the Collatz--Wielandt formula to the irreducible nonnegative matrix $\Lambda A$ with $\bm{x}=\bm{1}$:
\[
\rho(\Lambda A)=\max_{\bm{x}\gg 0}\min_i \frac{(\Lambda A\bm{x})_i}{x_i}
\;\ge\;
\min_i \frac{(\Lambda A\bm{1})_i}{1}
=
\min_i \lambda_i k_i.
\]
Dividing both bounds by $\delta$ yields~\eqref{eq:R0_degree_bounds}.
\end{proof}

\begin{corollary}[Absolute bounds] \label{cor:R0_abs_bounds}
For any facility configuration $L$,
\[
\frac{\beta_2}{\delta}\,\rho(A)\;\le\;\mathcal R_0(L)\;\le\;\frac{\beta_1}{\delta}\,\rho(A).
\]
\end{corollary}

\begin{proof}
By Lemma~\ref{lem:lambda_properties}, $\beta_2\le \lambda_i\le \beta_1$ for all $i$, so $\lambda_{\min}\ge \beta_2$ and $\lambda_{\max}\le \beta_1$.  Substitute into Proposition~\ref{prop:R0_bounds}.
\end{proof}
...
\begin{corollary}[Monotonicity in epidemiological parameters]\label{cor:R0_param_monotone}
Fix the network $A$ and a facility configuration $L$. Then $\mathcal R_0(L)=\rho(\Lambda A)/\delta$ is:
(i) strictly decreasing in $\delta$,
(ii) nondecreasing in $\beta_1$ and $\beta_2$, and
(iii) nondecreasing in $\omega$ (strict if $\nu_i(L)>0$ for some $i$).
\end{corollary}

\begin{proof}
The dependence on $\delta$ is explicit: $\mathcal R_0(L)=\rho(\Lambda A)/\delta$.
For $\beta_1,\beta_2,\omega$, note from Lemma~\ref{lem:lambda_properties} that $\partial \lambda_i/\partial \beta_1\ge 0$, $\partial \lambda_i/\partial \beta_2\ge 0$, and $\partial \lambda_i/\partial \omega\ge 0$.  Since $\Lambda A$ is nonnegative, increasing any diagonal entry of $\Lambda$ increases $\rho(\Lambda A)$ by Perron--Frobenius monotonicity, hence the stated monotonicities for $\mathcal R_0(L)$.
\end{proof}

\begin{proposition}[A sufficient coverage condition for $\mathcal R_0(L)<1$]\label{prop:coverage_sufficient}
Assume every node lies within distance $D$ of an open facility, i.e., $\max_i d_i(L)\le D$.
Define
\[
\underline{\nu}(D)=v_{\min}+(v_{\max}-v_{\min})e^{-\alpha D}.
\]
Then $\nu_i(L)\ge \underline{\nu}(D)$ for all $i$, and hence $\lambda_i(L)\le \overline{\lambda}(D)$ for all $i$, where
\begin{equation}\label{eq:lambda_bar_D}
\overline{\lambda}(D)=\frac{\beta_1\omega+\beta_2\underline{\nu}(D)}{\underline{\nu}(D)+\omega}.
\end{equation}
Consequently,
\begin{equation}\label{eq:R0_cover_bound}
\mathcal R_0(L)\le \frac{\overline{\lambda}(D)}{\delta}\,\rho(A).
\end{equation}
In particular, a sufficient condition for disease elimination is $\overline{\lambda}(D)\,\rho(A)<\delta$.
\end{proposition}

\begin{proof}
If $\max_i d_i(L)\le D$, then $d_i(L)\le D$ for all $i$, and since $d\mapsto e^{-\alpha d}$ is decreasing, we have $e^{-\alpha d_i(L)}\ge e^{-\alpha D}$. Hence
\[
\nu_i(L)=v_{\min}+(v_{\max}-v_{\min})e^{-\alpha d_i(L)}\ge v_{\min}+(v_{\max}-v_{\min})e^{-\alpha D}=\underline{\nu}(D).
\]
By~\eqref{eq:lambda_deriv}, $\lambda_i$ is decreasing in $\nu_i$ (for $\omega>0$), so $\nu_i(L)\ge \underline{\nu}(D)$ implies $\lambda_i(L)\le \overline{\lambda}(D)$ where $\overline{\lambda}(D)$ is~\eqref{eq:lambda_bar_D}.  Then Proposition~\ref{prop:R0_bounds} gives
\[
\mathcal R_0(L)=\frac{1}{\delta}\rho(\Lambda A)\le \frac{1}{\delta}\lambda_{\max}\rho(A)\le \frac{1}{\delta}\overline{\lambda}(D)\rho(A),
\]
which is~\eqref{eq:R0_cover_bound}.
\end{proof}

\begin{theorem}[Spectral sensitivity of $\mathcal R_0$]\label{thm:R0_sensitivity}
Let $M=\Lambda A$ be irreducible and let $\rho=\rho(M)$. 
Let $\bm v\gg 0$ and $\bm u\gg 0$ be the right/left Perron eigenvectors,
\[
M\bm v=\rho\bm v,
\qquad
\bm u^\top M=\rho\bm u^\top,
\]
normalized so that $\bm u^\top \bm v=1$. Then for each $i$,
\begin{equation}\label{eq:drho_dlambda}
\frac{\partial \rho(\Lambda A)}{\partial \lambda_i}
= u_i\,(A\bm v)_i,
\qquad\text{and hence}\qquad
\frac{\partial \mathcal R_0}{\partial \lambda_i}
=\frac{1}{\delta}\,u_i\,(A\bm v)_i.
\end{equation}
\end{theorem}

\begin{proof}
Since $M$ is irreducible and nonnegative, $\rho$ is a simple eigenvalue (Perron--Frobenius), so standard eigenvalue perturbation theory applies. For a perturbation $\Delta M$, the first-order change in $\rho$ is
\[
\Delta \rho = \bm u^\top (\Delta M)\bm v + o(\|\Delta M\|),
\]
under the normalization $\bm u^\top \bm v=1$.
Here $M(\lambda)=\Lambda A$ and $\partial M/\partial \lambda_i = E_{ii}A$, where $E_{ii}$ is the matrix with a $1$ in entry $(i,i)$ and zeros elsewhere. Therefore,
\[
\frac{\partial \rho}{\partial \lambda_i}
=\bm u^\top (E_{ii}A)\bm v
= u_i\,(A\bm v)_i,
\]
which gives~\eqref{eq:drho_dlambda}. Dividing by $\delta$ yields the formula for $\partial \mathcal R_0/\partial \lambda_i$.
\end{proof}

\begin{corollary}[Sensitivity to vaccination access]\label{cor:R0_access_sensitivity}
Under the assumptions of Theorem~\ref{thm:R0_sensitivity} and $\omega>0$,
\begin{align}
\frac{\partial \mathcal R_0}{\partial \nu_i}
&=\frac{1}{\delta}\,u_i\,(A\bm v)_i\,
\frac{(\beta_2-\beta_1)\omega}{(\nu_i+\omega)^2}
\;<\;0, \label{eq:dR0_dnu}\\[4pt]
\frac{\partial \mathcal R_0}{\partial d_i}
&=\frac{\partial \mathcal R_0}{\partial \nu_i}\cdot
\frac{\partial \nu_i}{\partial d_i}
=\frac{1}{\delta}\,u_i\,(A\bm v)_i\,
\frac{(\beta_1-\beta_2)\omega}{(\nu_i+\omega)^2}\,
\alpha(v_{\max}-v_{\min})e^{-\alpha d_i}
\;>\;0. \label{eq:dR0_dd}
\end{align}
\end{corollary}

\begin{proof}
Equation~\eqref{eq:dR0_dnu} follows from the chain rule:
\[
\frac{\partial \mathcal R_0}{\partial \nu_i}
=
\frac{\partial \mathcal R_0}{\partial \lambda_i}
\frac{\partial \lambda_i}{\partial \nu_i},
\]
using~\eqref{eq:drho_dlambda} and~\eqref{eq:lambda_deriv}.  For~\eqref{eq:dR0_dd}, note that
\[
\frac{\partial \nu_i}{\partial d_i}
=-(v_{\max}-v_{\min})\alpha e^{-\alpha d_i},
\]
so multiplying by~\eqref{eq:dR0_dnu} yields~\eqref{eq:dR0_dd}.
\end{proof}

\begin{remark}[Implication for \ac{RL} guidance]\label{rem:R0_sensitivity_RL}
Corollary~\ref{cor:R0_access_sensitivity} implies that the marginal epidemiological value of improving access for node $i$ is proportional to the weight $u_i(A\bm v)_i$, which depends on the Perron left/right eigenvectors of $\Lambda A$.  This provides a principled, network-aware importance score that can be used to design baselines (e.g., open facilities that most reduce weighted distances) or to shape learning signals for \ac{RL}.
\end{remark}

\subsection{A Submodular Access Surrogate (Facility-Location Baseline)} \label{subsec:submodular_access}

While $\mathcal R_0(L)$ is the central epidemic threshold, it is nonlinear and costly to evaluate for all configurations. A useful surrogate for vaccination access is the exponential coverage score
\[
g(L)=\sum_{i=1}^n e^{-\alpha d_i(L)}.
\]

\begin{lemma}[Submodularity of exponential-access coverage]\label{lem:submod_access}
The set function $g:2^{\mathcal M}\to \mathbb R$ defined by
\[
g(L)=\sum_{i=1}^n e^{-\alpha d_i(L)}=\sum_{i=1}^n \max_{m\in L} e^{-\alpha\|z_i-m\|_2}
\]
(with the convention $\max_{m\in\emptyset}(\cdot)=0$) is monotone nondecreasing and submodular.
Consequently, $\sum_{i=1}^n \nu_i(L)=n v_{\min}+(v_{\max}-v_{\min})g(L)$ is also monotone submodular.
\end{lemma}

\begin{proof}
Fix $i$ and define $g_i(L)=\max_{m\in L} w_{im}$ where $w_{im}=e^{-\alpha\|z_i-m\|_2}\in(0,1]$ and $g_i(\emptyset)=0$.
Monotonicity is immediate since enlarging $L$ can only increase the maximum.
To show submodularity, let $L\subseteq L'$ and $m\notin L'$. The marginal gain satisfies
\[
g_i(L\cup\{m\})-g_i(L)=\max\{g_i(L),w_{im}\}-g_i(L)=\max\{0,w_{im}-g_i(L)\}.
\]
Since $g_i(L')\ge g_i(L)$, we have $w_{im}-g_i(L')\le w_{im}-g_i(L)$, hence
\[
g_i(L'\cup\{m\})-g_i(L')=\max\{0,w_{im}-g_i(L')\}\le \max\{0,w_{im}-g_i(L)\}=g_i(L\cup\{m\})-g_i(L).
\]
Thus $g_i$ is submodular; summing over $i$ preserves submodularity, so $g$ is submodular.
The affine transformation to $\sum_i \nu_i(L)$ preserves submodularity.
\end{proof}

\begin{corollary}[Greedy approximation under a facility budget]\label{cor:greedy_access}
If one imposes a cardinality constraint $|L|\le K$ and seeks to maximize $g(L)$, the standard greedy algorithm attains
\[
g(L_{\mathrm{greedy}})\ge (1-1/e)\,g(L^\star),
\]
where $L^\star$ is an optimal size-$K$ set.
\end{corollary}

\begin{proof}
This is the classical Nemhauser--Wolsey result for monotone submodular maximization under a cardinality constraint.
\end{proof}

\begin{remark}[Policy Interpretation] \label{rem:policy}
Proposition~\ref{prop:R0_monotone} provides a theoretical justification for opening vaccination facilities: enlarging the set of open facilities $L$ monotonically reduces $\mathcal{R}_0(L)$.  However, each facility incurs operational costs~$C_O$ per time step and transition costs~$f^+$ (opening) and~$f^-$ (closing).  The reproduction number $\mathcal{R}_0(L)$ quantifies the epidemiological benefit of a given configuration, while the cost model captures the associated economic burden.  The \ac{RL} agent seeks facility configurations that balance these competing objectives, achieving favorable epidemic outcomes at minimum total cost.  Lemma~\ref{lem:submod_access} additionally provides a tractable access surrogate and a provable greedy baseline under budgeted variants of the facility-location problem.
\end{remark}
...
\begin{enumerate}[(i)]
    ...
    \item If $\mathcal{R}_0(L) > 1$, there exists an endemic equilibrium $(S_i^{\star}, I_i^{\star}, V_i^{\star})$ with $I_i^{\star} > 0$ for all $i \in \mathcal{V}$.  The bifurcation at $\mathcal{R}_0(L) = 1$ is forward: the endemic branch emerges continuously as $\mathcal{R}_0(L)$ crosses unity from below.
\end{enumerate}
\end{theorem}
...
\emph{Forward bifurcation.}  As $\mathcal{R}_0(L) \downarrow 1$, the threshold $\varepsilon_0$ in~(c) can be taken arbitrarily small, and Part~(i) shows that no nontrivial equilibrium exists at $\mathcal{R}_0(L) = 1$.  Hence $\|\bm{I}^{\star}\| \to 0$ continuously as $\mathcal{R}_0(L) \to 1^+$, confirming a forward bifurcation.  For the homogeneous case, the explicit quadratic characterization in Proposition~\ref{prop:endemic_homogeneous} provides an independent verification.
\end{proof}
...
The facility-operation problem studied in this paper is a dynamic control problem in which the action at decision epoch $t$ is the set of open facilities $L_t\subseteq\mathcal M$. Facility decisions determine vaccination rates $\nu_i(L_t)$ and thereby influence the epidemic evolution during the subsequent decision interval. In addition, each decision incurs switching costs (opening/closing) and operating costs. 

This section presents an interpretable greedy baseline that explicitly accounts for all costs---opening/closing, operating, vaccination (counted as $S\to V$ vaccination administrations), and infection burden---while avoiding expensive Monte Carlo rollouts by using a fast spectral proxy derived from the analytical threshold $\mathcal R_0(L)$. A key methodological contribution is that we use the analytical sensitivity results in Theorem~\ref{thm:R0_sensitivity} and Corollary~\ref{cor:R0_access_sensitivity} to \emph{order and screen} candidate facility toggles, so that the greedy search evaluates the most epidemiologically influential moves first. Finally, we introduce a \ac{PPO} algorithm that uses the greedy baseline as an expert (imitation target) for accelerated learning, and subsequently surpasses it by optimizing the full long-horizon objective.

\subsection{Fast Spectral-Proxy H-step Greedy Rollout Baseline Policy} \label{subsec:GreedySpectralRollout}

Let $t=1,2,\dots,T$ index discrete decision epochs, and let $\Delta>0$ denote the duration of one decision interval. Let $\xi_t$ denote the simulator state at the beginning of epoch $t$ (e.g., the node-level configuration $\bm X^t=(X_1^t,\dots,X_n^t)$). The action at epoch $t$ is the facility set $L_t\subseteq\mathcal M$. We also keep track of the previously open set $L_{t-1}$, since switching costs depend on changes relative to the prior configuration.

Given $(\xi_t,L_{t-1})$ and an action $L_t$, we define the per-interval cost
\begin{equation}\label{eq:stage_cost_greedy}
c_t \;=\;
f^+\bigl|L_t\setminus L_{t-1}\bigr| \;+\;
f^-\bigl|L_{t-1}\setminus L_t\bigr|
\;+\;
C_O |L_t|
\;+\;
C_V\,N_{SV}^{(t)}
...
C_I\,N_I^{(t)} .
\end{equation}
Here $f^+$ and $f^-$ are one-time opening/closing costs, $C_O$ is the operating cost per open facility per interval, $N_{SV}^{(t)}$ is the number of vaccination administrations (state transitions $S\to V$ due to vaccination) during $[t,t+\Delta)$, and $N_I^{(t)}$ is the infection burden over that interval (e.g., end-of-interval prevalence $\sum_i \mathbf 1_{\{X_i^{t+\Delta}=I\}}$ or an integrated person-time burden).

Because vaccination cost is charged per $S\to V$ vaccination event, we approximate the expected number of vaccination administrations in the next interval under a fixed facility set $L$ using competing exponential hazards (vaccination vs.\ infection) held constant over $[t,t+\Delta)$. If $X_i^t=S$, define the instantaneous hazards
\[
h_{i}^{V}(L)=\nu_i(L),
\qquad
h_i^{I}(t)=\beta_1\,N_i^I(t),
\qquad
N_i^I(t)=\sum_{j\in\mathcal N_i}\mathbf 1_{\{X_j^t=I\}}.
\]
Approximating $h_i^{V}(L)$ and $h_i^{I}(t)$ as constant over the interval, the probability that node $i$ experiences a vaccination administration before infection within $\Delta$ is
\begin{equation}\label{eq:pSV_proxy}
p_{i,SV}(L\,|\,\xi_t)
\;=\;
\frac{h_i^{V}(L)}{h_i^{V}(L)+h_i^{I}(t)}
\left(1-e^{-(h_i^{V}(L)+h_i^{I}(t))\Delta}\right),
\qquad (X_i^t=S),
\end{equation}
(with the convention $p_{i,SV}=1-e^{-h_i^V(L)\Delta}$ if $h_i^{I}(t)=0$). Thus, the expected vaccination administrations per interval are approximated by
\begin{equation}\label{eq:NSV_proxy}
\widehat N_{SV}(L\,|\,\xi_t)
\;=\;
\sum_{i:\,X_i^t=S} p_{i,SV}(L\,|\,\xi_t).
\end{equation}

To avoid Monte Carlo epidemic rollouts, we approximate the multi-step infection burden under a fixed facility set $L$ using the local linearization about the \ac{DFE}. Recall from Theorem~\ref{thm:DFE_stability} that
\[
\mathcal R_0(L)=\frac{1}{\delta}\rho(\Lambda(L)A),
\qquad
\Lambda(L)=\mathrm{diag}(\lambda_1(L),\dots,\lambda_n(L)),
\qquad
\lambda_i(L)=\frac{\beta_1\omega+\beta_2\nu_i(L)}{\nu_i(L)+\omega}.
\]
Define the dominant spectral quantity
\begin{equation}\label{eq:mu_def_method}
\mu(L)\;=\;\rho(\Lambda(L)A),
\qquad
r(L)\;=\;\mu(L)-\delta,
\qquad
g(L)\;=\;e^{r(L)\Delta}.
\end{equation}
Under the linearized infection dynamics $\dot{\bm \iota}=(\Lambda(L)A-\delta I)\bm\iota$, the dominant mode grows/decays at rate $r(L)$; consequently, over one decision interval the infection magnitude is approximated to scale by $g(L)$. Let $I_t$ denote the number of infected individuals at the start of epoch $t$:
\[
I_t \;=\;\sum_{i=1}^n \mathbf 1_{\{X_i^t=I\}}.
\]
We approximate the infection burden $\ell$ intervals ahead by geometric growth/decay:
\begin{equation}\label{eq:NI_proxy_method}
\widehat N_I^{(t+\ell)}(L\,|\,\xi_t)
\;=\;
\min\{n,\; I_t\,g(L)^\ell\},
\qquad \ell=0,1,\dots,H-1.
\end{equation}

Fix a discount factor $\gamma\in(0,1]$ and an $H$-step lookahead horizon. Under a \emph{hold} lookahead (the configuration is kept fixed as $L$ across the $H$ steps), a convenient untruncated approximation is
\begin{equation}\label{eq:Qhat_spectral_closed_method}
\widehat Q_t^{(H)}(L\,|\,\xi_t,L_{t-1})
\;\approx\;
f^+\bigl|L\setminus L_{t-1}\bigr|
+f^-\bigl|L_{t-1}\setminus L\bigr|
+\bigl(C_O|L|+C_V\widehat N_{SV}(L\,|\,\xi_t)\bigr)S_H(\gamma)
+C_I I_t\,S_H(\gamma g(L)),
\end{equation}
where $S_H(x)=\sum_{\ell=0}^{H-1}x^\ell$.

\subsubsection{Sensitivity-guided ordering and screening of facility toggles} \label{subsubsec:sensitivity_ordering}

A naive greedy local search evaluates all facility toggles in an arbitrary order. In contrast, we exploit Theorem~\ref{thm:R0_sensitivity} and Corollary~\ref{cor:R0_access_sensitivity} to evaluate toggles in a principled order: we prioritize moves with the largest predicted marginal reduction in $\mu(L)=\rho(\Lambda(L)A)$ (equivalently, in $\mathcal R_0(L)$), since $\mu(L)$ controls the compounding infection term $S_H(\gamma g(L))$ in~\eqref{eq:Qhat_spectral_closed_method}.

Let $\bm v\gg 0$ and $\bm u\gg 0$ denote the right/left Perron eigenvectors of $\Lambda(L)A$,
\[
\Lambda(L)A\,\bm v=\mu(L)\bm v,
\qquad
\bm u^\top \Lambda(L)A=\mu(L)\bm u^\top,
\qquad
\bm u^\top \bm v=1.
\]
By Theorem~\ref{thm:R0_sensitivity} and Corollary~\ref{cor:R0_access_sensitivity}, the sensitivity of $\mu(L)$ to a marginal change in the access distance $d_i(L)$ is
\begin{equation}\label{eq:dmu_ddi}
\frac{\partial \mu(L)}{\partial d_i}
=
u_i\,(A\bm v)_i\;
\frac{(\beta_1-\beta_2)\omega}{(\nu_i(L)+\omega)^2}\;
\alpha\,(v_{\max}-v_{\min})e^{-\alpha d_i(L)}
\;>\;0.
\end{equation}
Define the \emph{nodal importance weight}
\begin{equation}\label{eq:kappa_i_def}
\kappa_i(L)\;:=\;\frac{\partial \mu(L)}{\partial d_i}.
\end{equation}
Nodes with large $\kappa_i(L)$ are those for which improving access (reducing $d_i$) yields the largest first-order reduction in $\mu(L)$ and hence in the predicted infection growth factor $g(L)$.

Let $d_{im}=\|z_i-m\|_2$ denote the distance from node $i$ to facility $m$. For the current configuration $L$, define $d_i(L)=\min_{m\in L} d_{im}$. Consider toggling a facility $m$:
\begin{itemize}
    \item \textbf{Opening move ($m\notin L$):} the new distance becomes $d_i(L\cup\{m\})=\min\{d_i(L),d_{im}\}$, so
    \begin{equation}\label{eq:Delta_d_open}
    \Delta d_i^{\mathrm{open}}(m)
    :=
    d_i(L\cup\{m\})-d_i(L)
    =
    \min\{d_i(L),d_{im}\}-d_i(L)
    =
    -\bigl[d_i(L)-d_{im}\bigr]_+,
    \end{equation}
    where $[x]_+=\max\{x,0\}$.
    \item \textbf{Closing move ($m\in L$):} the new distance becomes $d_i(L\setminus\{m\})$, so
    \begin{equation}\label{eq:Delta_d_close}
    \Delta d_i^{\mathrm{close}}(m)
    :=
    d_i(L\setminus\{m\})-d_i(L)
    \;\ge\;0.
    \end{equation}
    In implementation, $\Delta d_i^{\mathrm{close}}(m)$ can be computed efficiently by maintaining, for each node $i$, its nearest and second-nearest open facilities.
\end{itemize}
Using the first-order approximation $\Delta\mu \approx \sum_i (\partial\mu/\partial d_i)\Delta d_i$, we obtain the facility-level sensitivity score:
\begin{equation}\label{eq:Delta_mu_facility}
\Delta \mu_m
\;\approx\;
\sum_{i=1}^n \kappa_i(L)\,\Delta d_i(m),
\end{equation}
where $\Delta d_i(m)$ equals $\Delta d_i^{\mathrm{open}}(m)$ for an opening move and $\Delta d_i^{\mathrm{close}}(m)$ for a closing move.

For each candidate toggle $m$, we compute the first-order estimate $\Delta\mu_m$ via~\eqref{eq:Delta_mu_facility} and form an \emph{approximate} post-toggle spectral value
\begin{equation}\label{eq:mu_tilde_toggle}
\widetilde\mu(L\triangle\{m\})
\;:=\;
\mu(L)+\Delta\mu_m,
\qquad
\widetilde g(L\triangle\{m\})
\;:=\;
\exp\!\bigl((\widetilde\mu(L\triangle\{m\})-\delta)\Delta\bigr).
\end{equation}
We then compute a cheap screened objective
\begin{equation}\label{eq:Qtilde_screen}
\widetilde Q_t^{(H)}(L\triangle\{m\}\,|\,\xi_t,L_{t-1})
=
f^+\bigl|(L\triangle\{m\})\setminus L_{t-1}\bigr|
+f^-\bigl|L_{t-1}\setminus (L\triangle\{m\})\bigr|
+\bigl(C_O|L\triangle\{m\}|+C_V\widehat N_{SV}(L\triangle\{m\}\,|\,\xi_t)\bigr)S_H(\gamma)
+C_I I_t\,S_H\!\bigl(\gamma \widetilde g(L\triangle\{m\})\bigr),
\end{equation}
and evaluate toggles in ascending order of $\widetilde Q_t^{(H)}$. Intuitively, opening candidates that substantially reduce weighted distances for high-$\kappa_i(L)$ nodes are examined first, while closing candidates that substantially increase weighted distances are deprioritized unless their cost savings dominate.

\subsubsection{H-step greedy local search with sensitivity-ordered toggle evaluation} \label{subsubsec:greedy_algorithm}

Let $L\triangle\{m\}$ denote symmetric difference (toggle facility $m$). Starting from $L_{t-1}$, we repeatedly apply the single toggle that most reduces the $H$-step proxy value and stop when no single toggle improves it. The sensitivity-guided ordering above determines \emph{which toggles are considered first}; optionally, we can restrict evaluation to the top $K_{\mathrm{eval}}$ toggles per iteration for additional speed (setting $K_{\mathrm{eval}}=|\mathcal M|$ recovers full evaluation).

\begin{algorithm}[t]
\caption{Sensitivity-ordered fast $H$-step greedy policy (spectral proxy)}
\label{alg:H_step_greedy_spectral_sensitivity}
\begin{algorithmic}[1]
\Require current state $\xi_t$; previous set $L_{t-1}$; horizon $H$; discount $\gamma$; tolerance $\varepsilon\ge 0$; evaluation budget $K_{\mathrm{eval}}\in\{1,\dots,|\mathcal M|\}$
\State $L \gets L_{t-1}$
\State Compute $\widehat Q \gets \widehat Q_t^{(H)}(L\,|\,\xi_t,L_{t-1})$ using~\eqref{eq:Qhat_spectral_closed_method} (with $\mu(L)$ computed by power iteration)
\Repeat
    \State Compute Perron eigenvectors $(\bm u,\bm v)$ of $\Lambda(L)A$ and nodal weights $\kappa_i(L)$ via~\eqref{eq:dmu_ddi}--\eqref{eq:kappa_i_def}
    \ForAll{$m\in\mathcal M$}
        \State Compute $\Delta d_i(m)$ (open/close) using~\eqref{eq:Delta_d_open}--\eqref{eq:Delta_d_close}
        \State $\Delta\mu_m \gets \sum_{i=1}^n \kappa_i(L)\,\Delta d_i(m)$ \Comment{first-order change in $\mu$}
        \State $\widetilde Q_m \gets \widetilde Q_t^{(H)}(L\triangle\{m\}\,|\,\xi_t,L_{t-1})$ using~\eqref{eq:Qtilde_screen}
    \EndFor
    \State Sort facilities to obtain an ordered list $(m_{(1)},m_{(2)},\dots)$ with $\widetilde Q_{m_{(1)}}\le \widetilde Q_{m_{(2)}}\le\cdots$
    \State $\widehat Q_{\min}\gets \widehat Q$; \quad $m^\star\gets \emptyset$
    \For{$k=1$ to $K_{\mathrm{eval}}$}
        \State $m\gets m_{(k)}$; \quad $L_m \gets L\triangle\{m\}$
        \State Compute $\widehat Q_m \gets \widehat Q_t^{(H)}(L_m\,|\,\xi_t,L_{t-1})$ using~\eqref{eq:Qhat_spectral_closed_method}
        \If{$\widehat Q_m < \widehat Q_{\min}$}
            \State $\widehat Q_{\min}\gets \widehat Q_m$; \quad $m^\star\gets m$
        \EndIf
    \EndFor
    \If{$\widehat Q_{\min} \le \widehat Q - \varepsilon$}
        \State $L \gets L\triangle\{m^\star\}$; \quad $\widehat Q \gets \widehat Q_{\min}$
    \EndIf
\Until{$\widehat Q_{\min} > \widehat Q - \varepsilon$}
\State \Return $L_t \gets L$
\end{algorithmic}
\end{algorithm}

The ordering induced by~\eqref{eq:dmu_ddi}--\eqref{eq:Qtilde_screen} prioritizes facility toggles that most reduce the dominant spectral quantity $\mu(L)=\rho(\Lambda(L)A)$, which in turn most reduces the compounding infection proxy $S_H(\gamma g(L))$ over the $H$-step horizon. Empirically this produces strong greedy actions while substantially reducing computation when $K_{\mathrm{eval}}\ll|\mathcal M|$.

Algorithm~\ref{alg:H_step_greedy_spectral_sensitivity} defines a structured expert policy with a transparent decision rule. Because sensitivity ordering makes the search consistent and focuses on epidemiologically meaningful moves, it provides a stable expert behavior that is well-suited for imitation (e.g., behavior cloning initialization or auxiliary imitation losses) before \ac{PPO} continues training to exceed the expert by optimizing the full long-horizon stochastic objective.
