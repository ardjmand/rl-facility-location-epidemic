...
The facility-operation problem studied in this paper is a dynamic control problem in which the action at decision epoch $t$ is the set of open facilities $L_t\subseteq\mathcal M$.  Facility decisions affect vaccination rates $\nu_i(L_t)$ and thereby influence the epidemic evolution during the subsequent decision interval.  In addition, each decision incurs switching costs (opening/closing) and operating costs.  This section presents an interpretable greedy baseline that explicitly accounts for all costs---opening/closing, operating, vaccination (counted as $S\to V$ vaccination administrations), and infection burden---while avoiding expensive Monte Carlo rollouts by using a fast \emph{spectral proxy} derived from the analytical threshold $\mathcal R_0(L)$.


\subsection{Fast Spectral-Proxy H-step Greedy Rollout Baseline Policy} \label{subsec:GreedySpectralRollout}


Let $t=1,2,\dots,T$ index discrete decision epochs, and let $\Delta>0$ denote the duration of one decision interval.  Let $\xi_t$ denote the simulator state at the beginning of epoch $t$ (e.g., the full node-level configuration $\bm X^t=(X_1^t,\dots,X_n^t)$).  The action at epoch $t$ is the facility set $L_t\subseteq\mathcal M$.  We also keep track of the previously open set $L_{t-1}$, since switching costs depend on changes relative to the prior configuration.
...
\qquad (X_i^t=S),
\end{equation}
(with the convention $p_{i,SV}=1-e^{-h_i^V(L)\Delta}$ if $h_i^{I}(t)=0$).
Thus, the expected vaccination administrations per interval are approximated by
\begin{equation}\label{eq:NSV_proxy}
\widehat N_{SV}(L\,|\,\xi_t)
\;=\;
\sum_{i:\,X_i^t=S} p_{i,SV}(L\,|\,\xi_t).
\end{equation}
This proxy is fast to evaluate because it uses only the current infected-neighbor counts $N_i^I(t)$ and the vaccination rates $\nu_i(L)$ induced by $L$.
...
\]
We approximate the expected infection burden at $\ell$ intervals ahead by geometric growth/decay:
\begin{equation}\label{eq:NI_proxy}
\widehat N_I^{(t+\ell)}(L\,|\,\xi_t)
\;=\;
\min\{n,\; I_t\,g(L)^\ell\},
\qquad \ell=0,1,\dots,H-1.
\end{equation}
The truncation at $n$ prevents the proxy from exceeding the population size when $g(L)>1$.

Fix a discount factor $\gamma\in(0,1]$ and an $H$-step lookahead horizon.  For a candidate facility set $L$, we define the fast $H$-step proxy value
\begin{equation}\label{eq:Qhat_spectral}
\widehat Q_t^{(H)}(L\,|\,\xi_t,L_{t-1})
\;=\;
f^+\bigl|L\setminus L_{t-1}\bigr|
+f^-\bigl|L_{t-1}\setminus L\bigr|
+\sum_{\ell=0}^{H-1}\gamma^\ell\Big(
C_O|L| + C_V\,\widehat N_{SV}(L\,|\,\xi_t) + C_I\,\widehat N_I^{(t+\ell)}(L\,|\,\xi_t)
\Big).
\end{equation}
Under a \emph{hold} lookahead (the configuration is kept fixed as $L$ across the $H$ steps), the first two terms account for switching once at the current epoch, while operating and vaccination-administration costs scale linearly across the horizon, and infection burden compounds according to the spectral growth factor $g(L)$.  Using the geometric-sum notation
\[
S_H(x)=\sum_{\ell=0}^{H-1}x^\ell=
\begin{cases}
\dfrac{1-x^H}{1-x}, & x\neq 1,\\[6pt]
H, & x=1,
\end{cases}
\]
a convenient untruncated approximation (dropping the $\min\{n,\cdot\}$ in~\eqref{eq:NI_proxy}) is
\begin{equation}\label{eq:Qhat_spectral_closed}
\widehat Q_t^{(H)}(L\,|\,\xi_t,L_{t-1})
\;\approx\;
f^+\bigl|L\setminus L_{t-1}\bigr|
+f^-\bigl|L_{t-1}\setminus L\bigr|
...
\end{equation}
This closed-form score is extremely fast to evaluate and captures the key multi-step effect: if $\mu(L)<\delta$ (equivalently $\mathcal R_0(L)<1$), then $g(L)<1$ and predicted infections decay geometrically; if $\mu(L)>\delta$, then $g(L)>1$ and infection burden grows over the horizon.

Because $A$ is symmetric and $\Lambda(L)$ is diagonal with positive entries, $\mu(L)=\rho(\Lambda(L)A)$ equals the largest eigenvalue of the symmetric matrix $\Lambda(L)^{1/2}A\Lambda(L)^{1/2}$.  In practice, $\mu(L)$ can be approximated efficiently via a few iterations of power iteration, warm-started from the previous epoch's dominant eigenvector.  For additional speed, one may replace $\mu(L)$ by an upper bound (e.g., $\mu(L)\le \lambda_{\max}(L)\rho(A)$) when screening many toggles.

Let $L\triangle\{m\}$ denote the symmetric difference, i.e., toggling facility $m$ (open it if closed; close it if open).  Starting from $L_{t-1}$, we repeatedly apply the single-facility toggle that most reduces the proxy value~\eqref{eq:Qhat_spectral} and stop when no single toggle improves the score.  This yields a one-flip stable configuration that balances switching/operating costs against predicted vaccination and infection costs over an $H$-step horizon. Algorithm \label{alg:H_step_greedy_spectral} outlines the fast H-step greedy policy using the spectral proxy.

\begin{algorithm}[t]
\caption{Fast H-step greedy policy using the spectral proxy}
\label{alg:H_step_greedy_spectral}
\begin{algorithmic}[1]
\Require current simulator state $\xi_t$; previous facility set $L_{t-1}$; horizon $H$; discount $\gamma$; tolerance $\varepsilon\ge 0$
\State $L \gets L_{t-1}$
\State $J \gets \widehat Q_t^{(H)}(L\,|\,\xi_t,L_{t-1})$ using~\eqref{eq:Qhat_spectral} (or~\eqref{eq:Qhat_spectral_closed})
\Repeat
    \State $J_{\min}\gets J$; \quad $m^\star\gets \emptyset$
    \ForAll{$m\in\mathcal M$}
        \State $L_m \gets L\triangle\{m\}$
        \State $J_m \gets \widehat Q_t^{(H)}(L_m\,|\,\xi_t,L_{t-1})$
        \If{$J_m < J_{\min}$}
            \State $J_{\min}\gets J_m$; \quad $m^\star\gets m$
        \EndIf
    \EndFor
    \If{$J_{\min}\le J-\varepsilon$}
        \State $L \gets L\triangle\{m^\star\}$; \quad $J\gets J_{\min}$
    \EndIf
\Until{$J_{\min} > J-\varepsilon$}
\State \Return $L_t \gets L$
\end{algorithmic}
\end{algorithm}

Two implementation notes are worth mentioning: First, precomputing all pairwise distances $\|z_i-m\|_2$ enables fast updates of $d_i(L)$ and $\nu_i(L)$ under toggles.  Likewise, $N_i^I(t)$ can be computed once per epoch from $\xi_t$. Second, the spectral proxy is motivated by the \ac{DFE} linearization and is most accurate when infection levels are moderate and the system is near the \ac{DFE} regime.  Empirically, it provides a strong fast baseline and a useful ``expert'' behavior for imitation; \ac{PPO} can later improve upon it by optimizing the full long-horizon stochastic objective.

\begin{remark}[Using the greedy policy as an imitation target for \ac{PPO}] \label{rem:greedy_imitation_spectral}
Algorithm~\ref{alg:H_step_greedy_spectral} defines a deterministic expert mapping from the agent's observation at epoch $t$ (including $\xi_t$ and $L_{t-1}$) to an action $L_t$.
This expert can be used to generate a dataset of state--action pairs $\{(\mathrm{obs}_t, L_t^{\mathrm{greedy}})\}$ for behavior cloning initialization, or as an auxiliary imitation objective (e.g., cross-entropy/KL toward the expert) during \ac{PPO} training.  Because the expert uses the analytically grounded proxy $\mu(L)=\rho(\Lambda(L)A)$ to anticipate compounding infection costs over $H$ steps, it encodes a structured heuristic that \ac{PPO} can mimic and subsequently surpass by learning non-myopic switching behavior under the full simulator dynamics and cost model.
\end{remark}
